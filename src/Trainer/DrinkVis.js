...
export default function DrinkVis(props) {
  ...
  if (props.drinks.length > 0) {
    ...
    for (const [index, value] of props.curTraining.positions.entries()) {
      console.log(temp);
      circles.push(<circle className={temp[props.curTraining.id][index] ? "clicked" : "unclicked"} r="10" cx={props.curTraining["positions"][index].x} cy={props.curTraining["positions"][index].y} fill="#FFFFC9" stroke="black" strokeWidth="1" />)
      //add clear circle on top of circle and text to make text clickable as well
      clickableArea.push(<circle r="10" cx={props.curTraining["positions"][index].x} cy={props.curTraining["positions"][index].y} opacity="0" onClick={() => {
